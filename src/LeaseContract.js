...
const Contract = ({ contract_number, contract_status, house_address, billing_deadline, contract_date, 
...
class LeaseContract extends Component {
    ...
    searchLeaseContract = () => {
        ...
        Axios.post("http://localhost:3001/searchLeaseContract", { selected_date, start_date, end_date, house_id, lot_number, tenant_id, contract_status, billing_deadline })
            .then((response) => {
    ...
    render() {
        const { contractList, selected_date, house_id, lot_number, tenant_id, contract_status, billing_deadline } = this.state;
